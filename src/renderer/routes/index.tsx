...
import { YachiyoChatLanding } from '@/components/yachiyo/YachiyoChatLanding'
...
function Index() {
  ...
  const { copilots: remoteCopilots } = useRemoteCopilotsByCursor({ enabled: !inAndroidAppShell, limit: 10 })
  ...
  return (
    <Page title="">
      <div className="p-0 flex flex-col h-full">
        ...
        {!inAndroidAppShell && welcomeCardMode !== 'none' && (
          <Box px="sm">
            <ChatboxWelcomeCard
...
const CopilotPicker = ({ selectedId, onSelect }: { selectedId?: string; onSelect?(copilot?: CopilotDetail): void }) => {
  ...
  const widthFull = useUIStore((s) => s.widthFull)
  const { copilots: myCopilots } = useMyCopilots()
  const { copilots: remoteCopilots } = useRemoteCopilotsByCursor({ enabled: !inAndroidAppShell })
