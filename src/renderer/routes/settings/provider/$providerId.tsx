...
function ProviderSettings({ providerId }: { providerId: string }) {
  ...
  const isFixedYachiyoHost = baseInfo?.id === ModelProviderEnum.Yachiyo
  ...
  const normalizedBuiltinApiHost = baseInfo
    ? normalizeAPIHost(
        {
          ...providerSettings,
          apiHost: isFixedYachiyoHost
            ? YACHIYO_API_HOST
            : providerSettings?.apiHost || baseInfo.defaultSettings?.apiHost,
          apiPath: isFixedYachiyoHost
            ? baseInfo.defaultSettings?.apiPath
        ...
        {BUILTIN_API_HOST_PROVIDERS.has(baseInfo.id) && (
          <Stack gap="xxs" style={isOAuthActive ? { opacity: 0.5, pointerEvents: 'none' } : undefined}>
            ...
            <Flex gap="xs" align="center">
              <TextInput
                flex={1}
                value={isFixedYachiyoHost ? YACHIYO_API_HOST : providerSettings?.apiHost}
                placeholder={baseInfo.defaultSettings?.apiHost}
                onChange={isFixedYachiyoHost ? undefined : handleApiHostChange}
                readOnly={isFixedYachiyoHost}
              />
            </Flex>
