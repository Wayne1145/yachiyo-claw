...
import {
  decryptMobileSettings,
  encryptMobileSettings,
  hasYachiyoSecureStoragePrefix,
  isYachiyoSecureStorageEnvelope,
} from './native/yachiyo_secure_storage'

const PROTECTED_MOBILE_SETTINGS_ERROR_MESSAGE = 'Unable to read protected mobile settings.'

/**
 * Signals that only the encrypted mobile settings row is unreadable. The error deliberately
 * carries no cause so native Keystore details and protected values cannot escape this boundary.
 */
export class ProtectedMobileSettingsError extends Error {
  constructor() {
    super(PROTECTED_MOBILE_SETTINGS_ERROR_MESSAGE)
    this.name = 'ProtectedMobileSettingsError'
  }
}

export function isProtectedMobileSettingsError(error: unknown): error is ProtectedMobileSettingsError {
  return error instanceof ProtectedMobileSettingsError
}
...
export class MobileSQLiteStorage implements Storage {
  ...
  public async setStoreValue(key: string, value: any) {
    if (key === StorageKey.Settings) {
      ...
      try {
        ...
        throw new Error('Unable to serialize protected mobile settings.')
      }
      if (typeof serializedValue !== 'string') {
        throw new Error('Unable to serialize protected mobile settings.')
      }
      await this.sqliteStorage.setItem(key, await encryptMobileSettings(serializedValue))
      return
    }
    await this.sqliteStorage.setItem(key, JSON.stringify(value))
  }
  public async getStoreValue(key: string) {
    const storedValue = await this.sqliteStorage.getItem(key)
    if (!storedValue) return null
    return this.deserializeStoredValue(key, storedValue)
  }
  public async delStoreValue(key: string) {
  ...
  public async getAllStoreValues(): Promise<{ [key: string]: any }> {
    ...
    for (const key in items) {
      if (items[key] && typeof items[key] === 'string') {
        try {
          items[key] = await this.deserializeStoredValue(key, items[key])
        } catch (error) {
          if (key === StorageKey.Settings) {
            throw error
          }
  ...
  private async deserializeStoredValue(key: string, storedValue: string): Promise<any> {
    if (key !== StorageKey.Settings) {
    ...
    if (hasYachiyoSecureStoragePrefix(storedValue)) {
      if (!isYachiyoSecureStorageEnvelope(storedValue)) {
        throw new ProtectedMobileSettingsError()
      }
      try {
        return this.parseProtectedSettings(await decryptMobileSettings(storedValue))
      } catch {
        throw new ProtectedMobileSettingsError()
      }
    }

    // Parse before replacing the row so corrupt legacy data is never mistaken for a successful migration.
    const legacyValue = this.parseProtectedSettings(storedValue)
    try {
      const encryptedValue = await encryptMobileSettings(storedValue)
      await this.sqliteStorage.setItem(StorageKey.Settings, encryptedValue)
      return legacyValue
    } catch {
      throw new ProtectedMobileSettingsError()
    }
  ...
  private parseProtectedSettings(serializedValue: string): any {
    try {
