      ...
      })),
      {
        ...
        migrate: (persisted: any, version) => {
          ...
          switch (version) {
            ...
            case 4:
              if (!settings.defaultPrompt || settings.defaultPrompt === 'You are a helpful assistant.') {
                settings.defaultPrompt = YACHIYO_SOUL
              }
            case 5:
...
export async function persistSettingsPatch(nextSettings: Partial<Settings>): Promise<void> {
  const currentSettings = SettingsSchema.parse(settingsStore.getState())
  const persistedSettings = SettingsSchema.parse({ ...currentSettings, ...nextSettings })

  // Credential-bearing onboarding must observe the real Keystore/SQLite write,
  // while the regular store can continue using its debounced persistence path.
  await storage.setItemNow(StorageKey.Settings, {
    ...persistedSettings,
    __version: SETTINGS_STORAGE_VERSION,
  })
  settingsStore.getState().setSettings(nextSettings)
}
