...
describe('buildTaskSystemPrompt', () => {
  it('describes the working directory without claiming filesystem isolation', () => {
    const prompt = buildTaskSystemPrompt('/work/project')

    expect(prompt).toContain('Working directory: /work/project')
    expect(prompt).toContain('not a filesystem security boundary')
    expect(prompt).not.toContain('Blocked read paths')
  })
