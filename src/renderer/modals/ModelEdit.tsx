...
const ModelEdit = NiceModal.create((props: { model?: ProviderModelInfo; providerId?: string }) => {
  ...
  const [apiStyle, setApiStyle] = useState<ProviderModelInfo['apiStyle']>(props.model?.apiStyle || 'openai')
  ...
  useEffect(() => {
    ...
    setApiStyle(props.model?.apiStyle || 'openai')
  ...
  const handleSave = () => {
    modal.resolve({
      modelId,
      type,
      apiStyle: props.providerId === ModelProviderEnum.Yachiyo ? apiStyle : props.model?.apiStyle,
  ...
  return (
    ...
    >
      <Stack gap="md">
        ...
        {props.providerId === ModelProviderEnum.Yachiyo && type === 'chat' && (
          <Stack gap="xs">
            <Text fw="600">API 协议</Text>
            <AdaptiveSelect
              comboboxProps={{ withinPortal: false }}
              allowDeselect={false}
              data={[
              ...
              value={apiStyle}
              onChange={(value) => setApiStyle(value as ProviderModelInfo['apiStyle'])}
            />
            <Text size="xs" c="dimmed">
              Yachiyo API 会按所选协议调用同一模型；主机地址保持固定。
            </Text>
          </Stack>
        )}
