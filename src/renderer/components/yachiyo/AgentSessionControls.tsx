...
export function describeAgentMode(sessionId: string, enabled: boolean): string {
  ...
  return `Agent · ${backend} · ${APPROVAL_LABEL[config.approvalMode]}`
}
...
export function AgentSessionControls({
  ...
  const refreshBackend = async (selected = backend) => {
    ...
    if (selected === 'shizuku') {
      ...
      setBackendDetail(permissions.shizukuGranted ? 'Shizuku 已授权' : permissions.shizukuRunning ? '等待 Shizuku 授权' : 'Shizuku 服务未运行')
    } else {
  ...
  useEffect(() => {
    if (settingsOpened) void refreshBackend()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [settingsOpened, backend])
  ...
  return (
    <>
      <div className="yachiyo-agent-header-controls" data-enabled={enabled ? 'true' : 'false'}>
        ...
        <Text size="xs" c="dimmed" className="yachiyo-agent-mode-label">{status}</Text>
      </div>
      ...
      >
        <Stack gap="md">
          <AgentConfigurationPanel onBackendChange={(next) => setBackend(next)} />
          {!backendReady && (
            <Alert color="orange" icon={<IconAlertTriangle size={18} />} title="所选后端尚未就绪">
              <Text size="sm">{backendDetail}</Text>
              <Button mt="xs" size="compact-sm" variant="light" loading={authorizing} onClick={() => void authorizeBackend()}>
                {backend === 'root' ? '检测并授权 Root' : backend === 'shizuku' ? '授权 Shizuku' : '开启无障碍服务'}
              </Button>
            </Alert>
          )}
          ...
          <AdaptiveModal.Actions>
            <Button variant="default" onClick={() => setSettingsOpened(false)}>取消</Button>
            <Button loading={saving} disabled={!backendReady} onClick={() => void save()}>保存设置</Button>
          </AdaptiveModal.Actions>
        </Stack>
      </AdaptiveModal>
      ...
      >
        <Stack gap="md">
          ...
          <AdaptiveModal.Actions>
            <Button variant="default" onClick={() => setFullWarningOpened(false)}>保持审批</Button>
