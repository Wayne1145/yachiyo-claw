...
const InputBox = forwardRef<InputBoxRef, InputBoxProps>(
  (
    ...
    useEffect(() => {
      ...
      const assistant = [...currentSession.messages].reverse().find((message) => message.role === 'assistant')
      if (!assistant || assistant.generating || assistant.id === spokenAssistantRef.current) return
      spokenAssistantRef.current = assistant.id
      void speakText(getMessageText(assistant))
    }, [autoSpeak, currentSession])
    ...
    const { knowledgeBase, setKnowledgeBase } = useKnowledgeBase({ isNewSession, sessionId: currentSessionId })
