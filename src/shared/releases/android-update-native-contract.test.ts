...
describe('Android in-app update native contract', () => {
  ...
  it('exposes verified updates from filesDir and exports from cacheDir', () => {
    ...
    expect(paths['files-path']).toEqual({ name: 'verified_updates', path: 'verified-updates/' })
    const updater = readFileSync('android/app/src/main/java/io/github/yachiyoclaw/update/YachiyoUpdatePlugin.java', 'utf8')
    expect(updater).toContain('new File(getContext().getFilesDir(), UPDATE_DIR)')
  ...
  it('registers the updater and delegates installation to Android PackageManager', () => {
    ...
    expect(plugin).toContain('new Intent(Intent.ACTION_INSTALL_PACKAGE)')
    expect(plugin).toContain('FileProvider.getUriForFile')
    expect(plugin).toContain('ClipData.newRawUri')
    expect(plugin).toContain('grantUriPermission')
